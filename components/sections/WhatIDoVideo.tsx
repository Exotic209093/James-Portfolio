...
export default function WhatIDoVideo() {
  ...
  // Eyebrow fades in early and stays for the settled state.
  const eyebrowOpacity = useTransform(progress, [0, 0.08], [0, 1])
  const statementOpacity = useTransform(progress, [0.5, 0.7], [0, 1])
      ...
      <div className="relative z-10 h-full flex items-center justify-center px-4 sm:px-6">
        <div className="relative w-full max-w-5xl text-center">
          ...
          <div className="relative h-80 sm:h-96 mb-8">
            {TECH_TAGS.map((tag) => (
              <TechTag key={tag.label} progress={progress} {...tag} />
            ))}
