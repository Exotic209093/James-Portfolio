...
import { useVideoReveal } from './useVideoReveal'

// Animation phase markers, tied to the ink-droplet timeline. These used to be
// driven by scroll position; now the clip plays itself and publishes the same
// 0 → 1 progress, so the choreography is unchanged:
...
export default function Hero() {
  ...
  const { sectionRef, videoRef, progress } = useVideoReveal({ mode, targetSeconds: 5 })
  ...
  const subtitleOpacity = useReveal(progress, PHASES.dispersion.in, PHASES.dispersion.settled)
  const subtitleY = useRevealY(progress, PHASES.dispersion.in, PHASES.dispersion.settled)

  const taglineOpacity = useReveal(progress, PHASES.detail.in, PHASES.detail.settled)
  const taglineY = useRevealY(progress, PHASES.detail.in, PHASES.detail.settled)

  const ctasOpacity = useReveal(progress, PHASES.ctas.in, PHASES.ctas.settled)
  const ctasY = useRevealY(progress, PHASES.ctas.in, PHASES.ctas.settled)

  // Scroll cue fades in only once the intro has settled, inviting the next
  // section instead of inviting you to "scrub" the hero.
  const arrowOpacity = useTransform(progress, [0.8, 0.95], [0, 1])
      ...
      <div className="relative z-10 h-full flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="container mx-auto">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-4 relative">
...
// Reveal element opacity from 0 → 1 across a window, then hold.
